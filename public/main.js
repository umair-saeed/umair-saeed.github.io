...
window.onload = () => {
  ...
  function setChatElementAttrs(config) {
    let JWTURL = `${config.jwt_server_url || jwtServerUrl}?labFQDN=${config.labFQDN}&realm=${config.realm}&integrationId=${config.integrationId}&ttl=${config.ttl}&clientId=${config.clientId}&clientSecret=${config.clientSecret}`;
    chatElement.style.display = "block";
